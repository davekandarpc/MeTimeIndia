...
public class MultiSelectEditText<T extends HashTagModel> extends AppCompatEditText {
    ...
    final private Set<Integer> checkedIds = new HashSet<Integer>();
    ...
    private void addItemChecked(T item) {
        checkedIds.add(item.getTagId());
    }

    private void removeItemChecked(T item) {
        checkedIds.remove(item.getTagId());
    }

    private void setCheckedItems() {
        for (int i = 0; i < adapter.getCount(); i++) {
            T item = adapter.getItem(i);
            if (checkedIds.contains(item.getTagId()))
    ...
    protected List<T> filterData(final List<T> originalItems, final String lastCommaValue) {
        ...
        for (T item : originalItems) {
            if (item.getTagName().toLowerCase().startsWith(lastCommaValue.toLowerCase()))
                filtered.add(item);
        }
    ...
    public void removeItem(String itemName) {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (TextUtils.equals(item.getTagId().toString(), itemName)) {
                listView.setItemChecked(i, false);
    ...
    public void setString() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            sb.setSpan(new BubbleSpan(bd, item.getTagId().toString()), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
